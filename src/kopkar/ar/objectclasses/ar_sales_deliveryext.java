/*
 * To change this ar_sales_delivery, choose Tools | ar_sales_deliverys
 * and open the ar_sales_delivery in the editor.
 */
 ...
 * @author mtgmis30
 */
public class ar_sales_deliveryext extends ar_sales_deliverycls implements NavigatorObjectInt{
    ...
    public ar_sales_deliveryext() {
        this.setdocument_date(GlobalUtils.sessiondate);
    }

    public ar_sales_delivery_detail1extcol getDetail1() {
        return detail1;
    }

    public void setDetail1(ar_sales_delivery_detail1extcol detail1) {
        this.detail1 = detail1;
    }
    private ar_sales_delivery_detail1extcol detail1=new ar_sales_delivery_detail1extcol(null);
    ...
    public Boolean update() throws CodeException {
        ...
        try {
            if (insertsts){
                squery="call sp_ar_sales_delivery_insert( "
                        + "'"+this.getdocentry()+"', "
                        ...
                        for (i=0;i<this.detail1.size();i++){
                            ...
                            if (o.getstatus()==ar_sales_delivery_detail1ext.Status_Insert){
                ...
                squery="call sp_ar_sales_delivery_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_ar_sales_delivery_delete( "
                    + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from ar_sales_delivery where id='"+id+"'";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
            ...
            Logger.getLogger(ar_sales_deliveryext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_ar_sales_delivery_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    private Boolean fillme(ResultSet rs) throws SQLException{
        ...
        detail1=new ar_sales_delivery_detail1extcol(this.getdocentry());
        detail1.initdata();
        return true;
    }
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_ar_sales_delivery_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_ar_sales_delivery_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_ar_sales_delivery_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_ar_sales_delivery_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public void calculatetotal() {
        ...
        for (i=0;i<this.detail1.size();i++){
            ar_sales_delivery_detail1ext o = this.detail1.get(i);
            if (!o.getstatus().equals(ar_sales_delivery_detail1ext.Status_delete)){
    ...
    public void retrieve(ar_sales_orderext objref) {
        int i=0;
        ...
        for (i=0;i<objref.getDetail1().size();i++){
            ...
            if (o.isSelected()){
                ar_sales_delivery_detail1ext pod=new ar_sales_delivery_detail1ext();
