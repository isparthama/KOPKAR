/*
 * To change this in_goods_transfer_detail1, choose Tools | in_goods_transfer_detail1s
 * and open the in_goods_transfer_detail1 in the editor.
 */
 ...
 * @author mtgmis30
 */
public class in_goods_transfer_detail1ext extends in_goods_transfer_detail1cls implements NavigatorObjectInt{
    ...
    public Boolean update() throws CodeException {
        try {
            if (status==in_goods_transfer_detail1ext.Status_Insert){
                squery="call sp_in_goods_transfer_detail1_insert( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getline_number()+"', "
                ...
                squery="call sp_in_goods_transfer_detail1_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getline_number()+"', "
                        + "'"+this.getproduct_code()+"', "
                        + "'"+this.getuom_id()+"', "
                ...
                squery="call sp_in_goods_transfer_detail1_delete( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getline_number()+"', "
                        + "'"+this.getproduct_code()+"', "
                        + "'"+this.getuom_id()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from in_goods_transfer_detail1 where id='"+id+"'";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
            ...
            Logger.getLogger(in_goods_transfer_detail1ext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String docentry) throws CodeException {
        squery="CALL sp_in_goods_transfer_detail1_get('"+docentry+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_in_goods_transfer_detail1_get('"+this.getdocentry()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_in_goods_transfer_detail1_get('"+this.getdocentry()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_in_goods_transfer_detail1_get('"+this.getdocentry()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_in_goods_transfer_detail1_get('"+this.getdocentry()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
