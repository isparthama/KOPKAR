/*
 * To change this in_goods_transfer, choose Tools | in_goods_transfers
 * and open the in_goods_transfer in the editor.
 */
 ...
 * @author mtgmis30
 */
public class in_goods_transferext extends in_goods_transfercls implements NavigatorObjectInt{
    ...
    public in_goods_transferext() {
        this.setdocument_date(GlobalUtils.sessiondate);
    }

    public in_goods_transfer_detail1extcol getDetail1() {
        return detail1;
    }

    public void setDetail1(in_goods_transfer_detail1extcol detail1) {
        this.detail1 = detail1;
    }
    private in_goods_transfer_detail1extcol detail1=new in_goods_transfer_detail1extcol(null);
    ...
    public Boolean update() throws CodeException {
        ...
        in_goods_transfer_detail1ext o;
        try {
            if (insertsts){
                squery="call sp_in_goods_transfer_insert( "
                        + "'"+this.getdocentry()+"', "
                        ...
                        + "'"+this.getprice_code()+"', "
                        + "'"+this.getwarehouse_code_fr()+"', "
                        + "'"+this.getwarehouse_code_to()+"', "
                        ...
                        for (i=0;i<this.detail1.size();i++){
                            ...
                            if (o.getstatus()==in_goods_transfer_detail1ext.Status_Insert){
                                o.setdocentry(this.getdocentry());
                            }
                            o.setwarehouse_code_fr(this.getwarehouse_code_fr());
                            o.setwarehouse_code_to(this.getwarehouse_code_to());
                ...
                squery="call sp_in_goods_transfer_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_in_goods_transfer_delete( "
                    + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from in_goods_transfer where id='"+id+"'";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
            ...
            Logger.getLogger(in_goods_transferext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_in_goods_transfer_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    private Boolean fillme(ResultSet rs) throws SQLException{
        ...
        this.setwarehouse_code_fr(rs.getString("warehouse_code_fr"));
        this.setwarehouse_code_to(rs.getString("warehouse_code_to"));
        ...
        detail1=new in_goods_transfer_detail1extcol(this.getdocentry());
        detail1.initdata();
        return true;
    }
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_in_goods_transfer_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_in_goods_transfer_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_in_goods_transfer_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_in_goods_transfer_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public void calculatetotal() {
        ...
        for (i=0;i<this.detail1.size();i++){
            in_goods_transfer_detail1ext o = this.detail1.get(i);
            if (!o.getstatus().equals(in_goods_transfer_detail1ext.Status_delete)){
