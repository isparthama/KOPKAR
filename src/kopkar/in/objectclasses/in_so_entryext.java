/*
 * To change this in_so_entry, choose Tools | in_so_entrys
 ...
 * @author mtgmis30
 */
public class in_so_entryext extends in_so_entrycls implements NavigatorObjectInt{
    Boolean insertsts=false;
    DbBean db=appsettinghandler.db;
    String squery="";
    private in_so_entry_detail1extcol detail;
    private String brand_name_fr;
    private String brand_name_to;
    private String category_name_fr;
    private String category_name_to;

    public in_so_entryext() {
        this.setdocentry(0);
        this.setdocument_date(GlobalUtils.sessiondate);
        this.setdocument_status("O");
        this.setcomplete_status("N");
        this.settotal_adm_qty(0);
        this.settotal_so_qty(0);
        detail=new in_so_entry_detail1extcol(this.getdocentry());
    }
    ...
    public Boolean update() throws CodeException {
        ...
        in_so_entry_detail1ext o;
        ResultSet rs;
        try {
            if (insertsts){
                squery="call sp_in_so_entry_insert( "
                        + "'"+this.getdocentry()+"', "
                ...
                squery="call sp_in_so_entry_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_in_so_entry_delete( "
                    + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from in_so_entry where id='"+id+"'";
        try {
            ...
            Logger.getLogger(in_so_entryext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_in_so_entry_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_in_so_entry_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_in_so_entry_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_in_so_entry_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_in_so_entry_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public in_so_entry_detail1extcol getdetail() {
        return this.detail;
    }
    ...
    public void calculatetotal() {
        ...
        Integer total_so_qty=new Integer(0);
        for (int i=0;i<detail.size();i++){
            in_so_entry_detail1ext o = detail.get(i);
            total_adm_qty+=o.getbase_total_pcs().intValue();
            total_so_qty+=o.gettotal_pcs().intValue();
        }
        this.settotal_adm_qty(total_adm_qty);
        this.settotal_so_qty(total_so_qty);
    }

    public boolean retrievedata(String sbrand_code_fr, String sbrand_code_to, String swarehouse_code) {
        try {
            if (this.detail.retrievedata(sbrand_code_fr, sbrand_code_to, swarehouse_code)) {
                calculatetotal();
                return true;
            }
        } catch (SQLException ex) {
