/*
 * To change this ko_transaksi_tagihan, choose Tools | ko_transaksi_tagihans
 ...
 * @author mtgmis30
 */
public class ko_transaksi_tagihanext extends ko_transaksi_tagihancls implements NavigatorObjectInt{
    Boolean insertsts=false;
    DbBean db=appsettinghandler.db;
    String squery="";
    protected ko_transaksi_tagihan_detail1extcol detail1;
    private String document_status_desc="Open";
    private double sisa_belum_dibayar=new Double(0);
    private String nama_departemen="";
    private int base_docentry;

    public ko_transaksi_tagihanext(){
        this.setdocument_date(GlobalUtils.sessiondate);
        detail1=new ko_transaksi_tagihan_detail1extcol(this);
    }
    ...
    public Boolean update() throws CodeException {
        ...
        try {
            if (insertsts){
                squery="call sp_ko_transaksi_tagihan_insert( "
                        + "'"+this.getdocentry()+"', "
                ...
                squery="call sp_ko_transaksi_tagihan_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_ko_transaksi_tagihan_delete( "
                    + "'"+this.getdocentry()+"', "
                    + "'"+this.getdocument_number()+"', "
                    + "'"+this.getcomplete_status()+"', "
                    + "'"+this.getdocument_date()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from ko_transaksi_tagihan where id='"+id+"'";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_ko_transaksi_tagihan_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    protected Boolean fillme(ResultSet rs) throws SQLException{
        ...
        this.detail1=new ko_transaksi_tagihan_detail1extcol(this);
        this.detail1.initdata();
        return true;
    }
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_ko_transaksi_tagihan_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_ko_transaksi_tagihan_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_ko_transaksi_tagihan_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_ko_transaksi_tagihan_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public void calculatetotal() {
        ...
        for (i=0;i<this.getDetail1().size();i++){
            ko_transaksi_tagihan_detail1ext o = this.getDetail1().get(i);
            if (!o.getstatus().equals(ko_transaksi_tagihan_detail1ext.Status_delete)&&o.getselected()){
     ...
     * @return the detail1
     */
    public ko_transaksi_tagihan_detail1extcol getDetail1() {
        return detail1;
    }
     ...
     */
    public void setDetail1(ko_transaksi_tagihan_detail1extcol detail1) {
    ...
    protected void setnama_departemen(String nama_departemen) {
        this.nama_departemen=nama_departemen;
    }

    public int getbase_docentry() {
        return this.base_docentry;
    }

    public void setbase_docentry(int base_docentry){
