/*
 * To change this ko_transaksi_setor_simpanan, choose Tools | ko_transaksi_setor_simpanans
...
import kopkar.ko.objectclasses.ko_transaksi_setor_simpanancls;
 ...
 * @author mtgmis30
 */
public class ko_transaksi_setor_simpananext extends ko_transaksi_setor_simpanancls implements NavigatorObjectInt{
    Boolean insertsts=false;
    DbBean db=appsettinghandler.db;
    String squery="";

    public ko_transaksi_setor_simpananext() {
        this.setmanual_sts("Y");
    ...
    public Boolean update() throws CodeException {
        ...
        try {
            if (insertsts){
                squery="call sp_ko_transaksi_setor_simpanan_insert( "
                ...
                squery="call sp_ko_transaksi_setor_simpanan_update( "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
                        + "'"+this.getmember_number()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_ko_transaksi_setor_simpanan_delete( "
                    + "'"+this.getdocument_number()+"', "
                    + "'"+this.getcomplete_status()+"', "
                    + "'"+this.getdocument_date()+"', "
                    + "'"+this.getmember_number()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from ko_transaksi_setor_simpanan where id='"+id+"'";
        try {
            ...
            Logger.getLogger(ko_transaksi_setor_simpananext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_ko_transaksi_setor_simpanan_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_ko_transaksi_setor_simpanan_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_ko_transaksi_setor_simpanan_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_ko_transaksi_setor_simpanan_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_ko_transaksi_setor_simpanan_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public boolean run_sp_ko_transaksi_setor_simpanan_auto_process(String tahun, String bulan) {
        try {
            db.execute("call sp_ko_transaksi_setor_simpanan_auto_process('"+tahun+"','"+bulan+"','"+GlobalUtils.sessiondate_mysql+"','"+GlobalUtils.audituser+"')");
            return true;
        } catch (SQLException ex) {
