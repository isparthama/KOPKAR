 ...
 * @author solution
 */
public class ko_transaksi_lembar_tagihan_detail1extcol extends ArrayList<ko_transaksi_lembar_tagihan_detail1ext> {
    ko_transaksi_lembar_tagihan_detail1ext data;
    String squery="";
    DbBean db=appsettinghandler.db;
    Integer id;

    ko_transaksi_lembar_tagihanext header;

    public void insert(){
        data= new ko_transaksi_lembar_tagihan_detail1ext(id);
    }
    public ko_transaksi_lembar_tagihan_detail1extcol(ko_transaksi_lembar_tagihanext header){
        this.header=header;
        this.id=this.header.getdocentry();
    }

    public ko_transaksi_lembar_tagihan_detail1ext getData() {
        return data;
    }

    public void setData(ko_transaksi_lembar_tagihan_detail1ext data) {
        this.data = data;
    }

    private Boolean fillme(ResultSet rs) throws SQLException, CodeException{
        if (rs.getString("pilih").equalsIgnoreCase("Y")||rs.getString("pilih").equalsIgnoreCase("1")){
...
//        data.setaudituser(rs.getString("audituser"));
    ...
    public void initdata() throws SQLException, CodeException{
        
        squery="call sp_ko_transaksi_lembar_tagihan_detail1_get("+header.getyop_tagihan()
                +","+header.getmop_tagihan()
                +","+header.getday_tagihan()
                +","+header.getdocentry()
        ...
        while (rs.next()){
            data=new ko_transaksi_lembar_tagihan_detail1ext(this.id);
            if (fillme(rs)){
                this.add(data);
            }
        }
    ...
    public Boolean delete(int row) {
        ko_transaksi_lembar_tagihan_detail1ext o = this.get(row);
        try {
            return o.delete();
        } catch (CodeException ex) {
            Logger.getLogger(ko_transaksi_lembar_tagihan_detail1extcol.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }
}
