/*
 * To change this ap_purchase_receive, choose Tools | ap_purchase_receives
 * and open the ap_purchase_receive in the editor.
 */
...
import jdbc.DbBean;
import kopkar.ap.objectclasses.ap_purchase_receivecls;
 ...
 * @author mtgmis30
 */
public class ap_purchase_receiveext extends ap_purchase_receivecls implements NavigatorObjectInt{
    ...
    public ap_purchase_receiveext() {
        this.setdocument_date(GlobalUtils.sessiondate);
    }

    public ap_purchase_receive_detail1extcol getDetail1() {
        return detail1;
    }

    public void setDetail1(ap_purchase_receive_detail1extcol detail1) {
        this.detail1 = detail1;
    }
    private ap_purchase_receive_detail1extcol detail1=new ap_purchase_receive_detail1extcol(null);
    ...
    public Boolean update() throws CodeException {
        ...
        try {
            if (insertsts){
                squery="call sp_ap_purchase_receive_insert( "
                        + "'"+this.getdocentry()+"', "
                        ...
                        for (i=0;i<this.detail1.size();i++){
                            ...
                            if (o.getstatus()==ap_purchase_receive_detail1ext.Status_Insert){
                ...
                squery="call sp_ap_purchase_receive_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_ap_purchase_receive_delete( "
                    + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from ap_purchase_receive where id='"+id+"'";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
            ...
            Logger.getLogger(ap_purchase_receiveext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_ap_purchase_receive_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    private Boolean fillme(ResultSet rs) throws SQLException{
        ...
        detail1=new ap_purchase_receive_detail1extcol(this.getdocentry());
        detail1.initdata();
        return true;
    }
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_ap_purchase_receive_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_ap_purchase_receive_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_ap_purchase_receive_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_ap_purchase_receive_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public void calculatetotal() {
        ...
        for (i=0;i<this.detail1.size();i++){
            ap_purchase_receive_detail1ext o = this.detail1.get(i);
            if (!o.getstatus().equals(ap_purchase_receive_detail1ext.Status_delete)){
    ...
    public void retrieve(ap_purchase_orderext objref) {
        int i=0;
        ...
        for (i=0;i<objref.getDetail1().size();i++){
            ...
            if (o.isSelected()){
                ap_purchase_receive_detail1ext pod=new ap_purchase_receive_detail1ext();
