/*
 * To change this ap_option, choose Tools | ap_options
 ...
 * @author mtgmis30
 */
public class ap_optionext extends ap_optioncls implements NavigatorObjectInt{
    ...
    private String TableName="ap_option";
    ...
    public Boolean update() throws CodeException {
        if (insertsts){
            squery="call sp_"+TableName+"_insert( "
                    + "'"+this.getcompany_id()+"', "
            ...
            squery="call sp_"+TableName+"_update( "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_"+TableName+"_delete( "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from ap_option where id='"+id+"'";
        try {
            ...
            Logger.getLogger(ap_optionext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String company_id) throws CodeException {
        squery="CALL sp_"+TableName+"_get('"+company_id+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_"+TableName+"_get('"+this.getcompany_id()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_"+TableName+"_get('"+this.getcompany_id()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_"+TableName+"_get('"+this.getcompany_id()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_"+TableName+"_get('"+this.getcompany_id()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
