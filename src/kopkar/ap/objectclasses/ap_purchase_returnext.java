/*
 * To change this ap_purchase_return, choose Tools | ap_purchase_returns
 * and open the ap_purchase_return in the editor.
 */
...
import jdbc.DbBean;
import kopkar.ap.objectclasses.ap_purchase_returncls;
 ...
 * @author mtgmis30
 */
public class ap_purchase_returnext extends ap_purchase_returncls implements NavigatorObjectInt{
    ...
    public ap_purchase_returnext() {
        this.setdocument_date(GlobalUtils.sessiondate);
    }

    public ap_purchase_return_detail1extcol getDetail1() {
        return detail1;
    }

    public void setDetail1(ap_purchase_return_detail1extcol detail1) {
        this.detail1 = detail1;
    }
    private ap_purchase_return_detail1extcol detail1=new ap_purchase_return_detail1extcol(null);
    ...
    public Boolean update() throws CodeException {
        ...
        try {
            if (insertsts){
                squery="call sp_ap_purchase_return_insert( "
                        + "'"+this.getdocentry()+"', "
                        ...
                        for (i=0;i<this.detail1.size();i++){
                            ...
                            if (o.getstatus()==ap_purchase_return_detail1ext.Status_Insert){
                ...
                squery="call sp_ap_purchase_return_update( "
                        + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean delete() throws CodeException {
        squery="call sp_ap_purchase_return_delete( "
                    + "'"+this.getdocentry()+"', "
                        + "'"+this.getdocument_number()+"', "
                        + "'"+this.getcomplete_status()+"', "
                        + "'"+this.getdocument_date()+"', "
    ...
    public Boolean initdata(Integer id) throws CodeException {
        squery="select * from ap_purchase_return where id='"+id+"'";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
            ...
            Logger.getLogger(ap_purchase_returnext.class.getName()).log(Level.SEVERE, null, ex);
        }
        return false;
    }

    public Boolean initdata(String document_number) throws CodeException {
        squery="CALL sp_ap_purchase_return_get('"+document_number+"', 0)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    private Boolean fillme(ResultSet rs) throws SQLException{
        ...
        detail1=new ap_purchase_return_detail1extcol(this.getdocentry());
        detail1.initdata();
        return true;
    }
    public Boolean moveFirst() throws CodeException {
        squery="CALL sp_ap_purchase_return_get('"+this.getdocument_number()+"', 1)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean movePrevious() throws CodeException {
        squery="CALL sp_ap_purchase_return_get('"+this.getdocument_number()+"', 2)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveNext() throws CodeException {
        squery="CALL sp_ap_purchase_return_get('"+this.getdocument_number()+"', 3)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public Boolean moveLast() throws CodeException {
        squery="CALL sp_ap_purchase_return_get('"+this.getdocument_number()+"', 4)";
        try {
            ResultSet rs = db.executeQuery(squery);
            while (rs.next()){
                if (fillme(rs)){
    ...
    public void calculatetotal() {
        ...
        for (i=0;i<this.detail1.size();i++){
            ap_purchase_return_detail1ext o = this.detail1.get(i);
            if (!o.getstatus().equals(ap_purchase_return_detail1ext.Status_delete)){
    ...
    public void retrieve(ap_purchase_receiveext objref) {
        ...
        this.setdscription("Base On PR No:"+objref.getdocument_number());
        for (i=0;i<objref.getDetail1().size();i++){
            ap_purchase_receive_detail1ext o = objref.getDetail1().get(i);
            if (o.isSelected()){
                ap_purchase_return_detail1ext pod=new ap_purchase_return_detail1ext();
